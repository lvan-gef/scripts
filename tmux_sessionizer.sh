...
# from the ThePrimeagen

# if [[ $# -eq 1 ]]; then
#     selected=$1
# else
#     selected=$(find /mnt/bulk2/code_projects ~/.config -mindepth 1 -maxdepth 1 -type d | fzf)
# fi
#
# if [[ -z $selected ]]; then
#     exit 0
...
[[ -n "$TS_SEARCH_PATHS" ]] || TS_SEARCH_PATHS=(/mnt/bulk2/code_projects ~/.config)
